...
# coding=utf8
import sys
sys.path.append('/home/aseem/ArmPi/')
import cv2
import time
import Camera
import threading
...
def initMove():
    ...
    AK.setPitchRangeMoving((0, 10, 10), -30, -30, -90, 750)
    ...
    while True:
        if __isRunning:
            if first_move and start_pick_up: # 当首次检测到物体时               
                ...
                setBuzzer(0.1)
                result = AK.setPitchRangeMoving((world_X, world_Y + 1.2, 10), -90, -90, 0, 750) # 不填运行时间参数，自适应运行时间
                if result == False:
                    unreachable = True
                else:
                    unreachable = False
                time.sleep(result[2]/1000) # 返回参数的第三项为时间
                #Board.setBusServoPulse(1, 0, 300)
                start_pick_up = False
                first_move = False
                action_finish = True
                ...
                if track: # 如果是跟踪阶段
                    if not __isRunning: # 停止以及退出标志位检测
                        continue
                    # AK.setPitchRangeMoving((world_X, world_Y + 1.2, 10), -90, -90, 0, 20)
                    # time.sleep(0.02)
                    # AK.setPitchRangeMoving((world_X, world_Y + 1.2, 10), -90, -90, 0, 1500)
                    # time.sleep(1.5)
                    AK.setPitchRangeMoving((world_X, world_Y + 1.2, 10), -90, -90, 0, 750)
                    track = False
                elif not start_pick_up:
                    if not valid_object_detected:
                        start_pick_up = True
                if start_pick_up: #如果物体没有移动一段时间，开始夹取
                    ...
                    if target_idx >= len(targets):
                       setTargetColor('None')
                        ...
                        setTargetColor(targets[target_idx])
                    time.sleep(0.1)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 1:
        targets = ['red', 'green', 'blue', 'green', 'red', 'green', 'blue']
    else:
        targets = sys.argv[1:]
    setTargetColor(targets[target_idx])
