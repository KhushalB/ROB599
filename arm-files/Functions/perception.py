...
class Perception:
    ...
    def run(self, img):
        ...
        areaMaxContour, area_max = self.get_contours(frame_lab)
        if area_max > 2500:  # have found the largest area
            ...
            cv2.putText(img, '(' + str(world_x) + ',' + str(world_y) + ')', (min(box[0, 0], box[2, 0]), box[2, 1] - 10),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, self.range_rgb[self.target_color], 1)  # draw center point
    ...
    def get_contours(self, frame_lab):
        area_max = 0
        areaMaxContour = 0
